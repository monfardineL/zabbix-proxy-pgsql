...
# Zabbix Proxy PostgreSQL Docker Entrypoint
...
create_db_schema_postgresql() {
    echo "** Checking if schema already exists in database '${DB_SERVER_DBNAME}'"
    
    # Try to get the version from the dbversion table directly
    ZBX_DB_VERSION=$(psql_query "SELECT mandatory FROM dbversion" "${DB_SERVER_DBNAME}")

    if [ -n "${ZBX_DB_VERSION}" ]; then
        echo "** Table '${DB_SERVER_DBNAME}.dbversion' already exists (Version: ${ZBX_DB_VERSION}). Skipping schema creation."
    else
...
update_zbx_config() {
    ...
    if [ -z "${ZBX_HOSTNAME}" ] && [ -n "${ZBX_HOSTNAMEITEM}" ]; then
        ...
        export ZBX_HOSTNAME="${ZBX_HOSTNAME:-"zabbix-proxy-postgresql"}"
    fi
    ...
    update_config_var "${ZABBIX_CONF_DIR}/zabbix_proxy.conf" "ProxyMode" "${ZBX_PROXYMODE}"
    update_config_var "${ZABBIX_CONF_DIR}/zabbix_proxy.conf" "Server" "${ZBX_SERVER_HOST}"
    update_config_var "${ZABBIX_CONF_DIR}/zabbix_proxy.conf" "ServerPort" "${ZBX_SERVER_PORT}"
    update_config_var "${ZABBIX_CONF_DIR}/zabbix_proxy.conf" "Hostname" "${ZBX_HOSTNAME}"
    update_config_var "${ZABBIX_CONF_DIR}/zabbix_proxy.conf" "HostnameItem" "${ZBX_HOSTNAMEITEM}"

    update_config_var "${ZABBIX_CONF_DIR}/zabbix_proxy.conf" "DBHost" "${ZBX_DB_HOST}"
    update_config_var "${ZABBIX_CONF_DIR}/zabbix_proxy.conf" "DBName" "${ZBX_DB_NAME}"
    update_config_var "${ZABBIX_CONF_DIR}/zabbix_proxy.conf" "DBSchema" "${ZBX_DB_SCHEMA}"
    update_config_var "${ZABBIX_CONF_DIR}/zabbix_proxy.conf" "DBUser" "${ZBX_DB_USER}"
    update_config_var "${ZABBIX_CONF_DIR}/zabbix_proxy.conf" "DBPassword" "${ZBX_DB_PASSWORD}"
    update_config_var "${ZABBIX_CONF_DIR}/zabbix_proxy.conf" "DBPort" "${ZBX_DB_PORT}"
    
    update_config_var "${ZABBIX_CONF_DIR}/zabbix_proxy.conf" "DebugLevel" "${ZBX_DEBUGLEVEL}"
    update_config_var "${ZABBIX_CONF_DIR}/zabbix_proxy.conf" "Timeout" "${ZBX_TIMEOUT}"
    update_config_var "${ZABBIX_CONF_DIR}/zabbix_proxy.conf" "AllowUnsupportedDBVersions" "${ZBX_ALLOWUNSUPPORTEDDBVERSIONS}"
    ...
    update_config_var "${ZABBIX_CONF_DIR}/zabbix_proxy.conf" "TLSConnect" "${ZBX_TLSCONNECT}"
    update_config_var "${ZABBIX_CONF_DIR}/zabbix_proxy.conf" "TLSAccept" "${ZBX_TLSACCEPT}"
    update_config_var "${ZABBIX_CONF_DIR}/zabbix_proxy.conf" "TLSCAFile" "${ZBX_TLSCAFILE}"
    update_config_var "${ZABBIX_CONF_DIR}/zabbix_proxy.conf" "TLSCRLFile" "${ZBX_TLSCRLFILE}"
    update_config_var "${ZABBIX_CONF_DIR}/zabbix_proxy.conf" "TLSServerCertIssuer" "${ZBX_TLSSERVERCERTISSUER}"
    update_config_var "${ZABBIX_CONF_DIR}/zabbix_proxy.conf" "TLSServerCertSubject" "${ZBX_TLSSERVERCERTSUBJECT}"
    update_config_var "${ZABBIX_CONF_DIR}/zabbix_proxy.conf" "TLSCertFile" "${ZBX_TLSCERTFILE}"
    update_config_var "${ZABBIX_CONF_DIR}/zabbix_proxy.conf" "TLSKeyFile" "${ZBX_TLSKEYFILE}"
    update_config_var "${ZABBIX_CONF_DIR}/zabbix_proxy.conf" "TLSPSKIdentity" "${ZBX_TLSPSKIDENTITY}"
    update_config_var "${ZABBIX_CONF_DIR}/zabbix_proxy.conf" "TLSPSKFile" "${ZBX_TLSPSKFILE}"
